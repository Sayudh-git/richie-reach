      ...
      <div className="overflow-x-auto">
        <div className="min-w-[700px]">
          ...
          <div className="grid grid-cols-[80px_repeat(7,1fr)] gap-1">
            <div />
            {DAYS.map((day) => (
              <div key={day} className="px-2 py-2 text-center font-mono text-[11px] text-muted-foreground">
                {day}
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-[80px_repeat(7,1fr)] gap-1">
            ...
            {DAYS.map((day) => {
              ...
              return (
                <div key={day} className="rounded border border-border bg-card px-2.5 py-2 min-h-[72px]">
                  <p className="text-xs font-medium text-foreground">{cell.label}</p>
                  <p className="mt-0.5 text-[10px] text-muted-foreground leading-snug">{cell.detail}</p>
                    ...
                    <div className="mt-1 space-y-1">
                      {FORK.branches.map((b, i) => (
                        <div key={i} className={`rounded px-1.5 py-0.5 text-[9px] ${
                          b.positive
                            ? 'bg-[#0A1A10] text-[#16A34A]'
                            : 'bg-[#0D0D0D] text-muted-foreground'
                        }`}>
